...
export interface ChartSpec {
  type: "bar" | "diverging";
  title: string;
  points: { label: string; value: number; warn?: boolean }[];
}

export interface AssistantAnswer {
  answer: string;
  /** Drawn from the same data the text states — never a separate computation. */
...
export interface DataStatus {
  for_sale_week: string | null;
  for_sale_count: number;
  sold_weeks_held: number;
  sold_total: number;
  sold_window: { week: string; sales: number; provisional: boolean }[];
  dealers: number;
  priced: number;
  oldest_week: string | null;
  newest_week: string | null;
}

/** The Jarvis opening — computed plays, no LLM involved. */
...
export interface Pulse {
  week_ending: string | null;
  stock: { now: number | null; delta: number | null };
  median_ask: { now: number | null; delta: number | null };
  median_age_days: { now: number | null; delta: number | null };
  sold_per_week: { now: number | null; delta: number | null; gap_week: boolean };
  weekly: { week: string; market: number; mine: number; gap: boolean }[];
  ages: { band: string; market_n: number; my_n: number }[];
  makes: { make: string; sales: number; my_sales: number; median_days: number | null }[];
}
