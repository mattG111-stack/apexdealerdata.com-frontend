...
function Delta({ label, now, delta, money: isMoney, invert, note }: {
  ...
  return (
    <div className="ax-stat" style={{ flex: "1 1 150px" }}>
...
const panel: React.CSSProperties = {
  ...
  padding: "16px 18px", background: "var(--panel)", boxShadow: "var(--shadow)",
};
...
function Greeting({ name }: { name: string | null }) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 29, fontWeight: 800, letterSpacing: "-.035em" }}>
...
function Evidence({ comps, confidence }: { comps: number | null; confidence: string | null }) {
  return (
    <span className={`ax-chip ${confidence || "low"}`}>
...
function Stat({ label, value, sub, tone }: {
  ...
  return (
    <div className={`ax-stat${tone === "warn" ? " warn" : ""}`}>
      <div className="ax-eyebrow">{label}</div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section style={{ marginTop: 36 }}>
      <h2 className="ax-eyebrow" style={{ marginBottom: 11 }}>{title}</h2>
      {children}
    </section>
  );
}
