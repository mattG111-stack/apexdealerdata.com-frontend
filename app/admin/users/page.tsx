...
export default function AdminUsersPage() {
  ...
  const [nu, setNu] = useState({ email: "", password: "", full_name: "", company: "" });
    ...
    <main style={{ maxWidth: 1080, margin: "0 auto", padding: "32px 20px" }}>
      ...
      <div style={{ marginTop: 16 }}>
        <button onClick={() => setShowNew((v) => !v)} style={{
          ...
          border: "1px solid #16A34A", background: "#22C55E", color: "#06210F",
          cursor: "pointer" }}>
          {showNew ? "Close" : "+ Add a user"}
        </button>
      </div>

      {showNew && (
        <div style={{ marginTop: 12, border: "1px solid #E3E7EE", borderRadius: 12,
                      background: "#fff", padding: 16 }}>
          <div style={{ display: "grid", gap: 8,
                        gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
            <input placeholder="Email" value={nu.email} type="email"
              onChange={(e) => setNu((v) => ({ ...v, email: e.target.value }))}
              style={inBox} />
            <input placeholder="Password" value={nu.password}
              onChange={(e) => setNu((v) => ({ ...v, password: e.target.value }))}
              style={inBox} />
            <input placeholder="Full name" value={nu.full_name}
              onChange={(e) => setNu((v) => ({ ...v, full_name: e.target.value }))}
              style={inBox} />
            <input placeholder="Dealership / company" value={nu.company}
              onChange={(e) => setNu((v) => ({ ...v, company: e.target.value }))}
              style={inBox} />
          </div>
          <div style={{ marginTop: 10, display: "flex", gap: 10, alignItems: "center" }}>
            <button
              disabled={busy || !nu.email || nu.password.length < 8}
              onClick={() => act(async () => {
                await api("/api/admin/users", {
                  ...
                  body: JSON.stringify({
                    ...
                    full_name: nu.full_name || null,
                    company: nu.company || null,
                  }),
                });
                setNu({ email: "", password: "", full_name: "", company: "" });
                setShowNew(false);
              })}
              style={{ fontSize: 13, fontWeight: 700, padding: "9px 18px", borderRadius: 8,
                       border: "1px solid #16A34A", background: "#22C55E", color: "#06210F",
                       cursor: "pointer",
                       opacity: busy || !nu.email || nu.password.length < 8 ? 0.5 : 1 }}>
              Create user
            </button>
            <span style={{ fontSize: 12.5, color: "#94A3B8" }}>
              Created approved and ready to sign in — assign their yard below, then
              hand them the email and password.
            </span>
          </div>
        </div>
      )}
...
const inBox: React.CSSProperties = {
  padding: "9px 12px", borderRadius: 8, border: "1px solid #D9DFE7", fontSize: 14,
};
