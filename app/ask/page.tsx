...
export default function AskPage() {
  ...
  return (
    <div className="ask-root">
      <a className="ask-back" href="/yard">← Your yard</a>
...
/** Particle orb. Idles slow, spins up while thinking. */
function Orb({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const p of parts) {
        p.a += p.s * (1 + boost * 3.4);
        const wob = Math.sin(p.a * 2.2 + p.z * 6) * (3 + boost * 6);
        const x = SIZE / 2 + Math.cos(p.a) * (p.r + wob);
        const y = SIZE / 2 + Math.sin(p.a) * (p.r + wob) * 0.82;

        // Tighter to the centre = brighter, so it reads as a core with a halo.
        const near = 1 - Math.min(p.r / 100, 1);
        const alpha = 0.10 + near * 0.55 + boost * 0.2;
        const hue = 168 + near * 26;
        ctx.fillStyle = `hsla(${hue}, 82%, ${52 + near * 26}%, ${alpha})`;
        ctx.fillRect(x, y, 1.5 + near * 1.2, 1.5 + near * 1.2);
      }

      const glow = ctx.createRadialGradient(SIZE / 2, SIZE / 2, 0, SIZE / 2, SIZE / 2, 66);
      glow.addColorStop(0, `rgba(90,240,200,${0.20 + boost * 0.20})`);
  ...
  return <canvas ref={ref} className="ask-orb" style={{ width: 260, height: 260 }} />;
}
