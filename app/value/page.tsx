...
export default function ValuePage() {
  ...
  return (
    <main className="apex" style={{ maxWidth: 900, margin: "0 auto", padding: "30px 20px 90px" }}>
      ...
      {result?.priced && (
        <>
          <div style={{ marginTop: 22, border: "1px solid #E3E7EE", borderRadius: 14,
          ...
          {result.comp_rows && result.comp_rows.length > 0 && (
            <>
              ...
              <div style={{ overflowX: "auto", border: "1px solid #E3E7EE",
                            borderRadius: 12, background: "#fff" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                  <thead><tr>
                    {["year","spec","region","kms","price","days","status"].map((h) => (
                      <th key={h} style={{ textAlign: "left", padding: "9px 13px",
                          fontSize: 11, textTransform: "uppercase", letterSpacing: ".05em",
                          color: "#94A3B8", borderBottom: "1px solid #EEF1F5" }}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {result.comp_rows.map((c, i) => (
                      <tr key={i}>
                        ...
                        <td style={td}>{c.variant ?? "—"}</td>
                        <td style={td}>{c.region ?? "—"}</td>
                        <td style={td}>{c.kms ? `${Math.round(c.kms / 1000)}k` : "—"}</td>
                        <td style={{ ...td, fontWeight: 700 }}>{money(c.price)}</td>
...
const td: React.CSSProperties = {
  padding: "9px 13px", borderBottom: "1px solid #F4F6F9", whiteSpace: "nowrap",
};
